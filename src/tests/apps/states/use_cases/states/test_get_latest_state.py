...
async def _execute(
    ...
    async with get_active_unit_of_work(
        ...
        media_storage_unit=InMemoryMediaStorageUnit(storage=media_store),
    ) as uow:
        states_media_storage = StatesMediaStorage(
            InMemoryMediaStorageBackend(unit_of_work=uow)
        )
        use_case = GetLatestStateUseCase(
...
async def test_get_latest_state_for_state_without_version(
    ...
    result = await _execute(
        active_transaction_session=active_transaction_session, state_name=state_name
    )
