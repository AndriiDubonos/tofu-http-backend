...
async def _execute(
    ...
    async with get_active_unit_of_work(active_db_session=active_transaction_session) as uow:
        use_case = UnlockStateUseCase(unit_of_work=uow, error_class=Exception)
...
async def test_unlock_state_for_non_existing_state(
    ...
    with pytest.raises(ValueError, match=f"State '{state_name}' is not locked"):
        await _execute(
...
async def test_unlock_state_with_incorrect_lock_id(
    ...
    with pytest.raises(ValueError, match=f"Cannot unlock state '{state_name}' with lock ID {incorrect_lock_id}"):
        await _execute(
            active_transaction_session=active_transaction_session,
            state_name=state_name,
