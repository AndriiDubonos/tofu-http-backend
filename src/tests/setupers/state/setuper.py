...
class StateSetuper:
    ...
    async def create_state(self, state_name: str) -> UUID:
        ...
        stmt = insert(StateTable).values(
            id=state_id, name=state_name, latest_version=None, lock_id=None
        )
    ...
    async def lock_state(self, state_name: str, lock_id: UUID = None) -> UUID:
        ...
        stmt = (
            update(StateTable).where(StateTable.id == state_id).values(lock_id=lock_id)
        )
        await self._session.execute(stmt)
        await self._session.flush()
    ...
    async def add_state_version(
        self, state_name: str, version: int, hash_value: str = None, path: str = None
    ) -> UUID:
        # Ensure state exists
        state_id = await self.create_state(state_name)
        ...
        stmt = (
            update(StateTable)
            .where(StateTable.id == state_id)
            .values(latest_version=version_id)
        )
        await self._session.execute(stmt)
        await self._session.flush()
    ...
    async def get_state(
        self, state_name: str
    ) -> tuple[StateTable | None, StateVersionTable | None]:
        # Get state with join to version
        stmt = (
            select(StateTable, StateVersionTable)
            .filter(StateTable.name == state_name)
            .outerjoin(
                StateVersionTable, StateTable.latest_version == StateVersionTable.id
            )
        )
