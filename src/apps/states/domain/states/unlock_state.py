...
def unlock_state(state: State, lock_id: UUID, force: bool) -> State:
    ...
    if state.lock_id is None:
        raise ValueError(f"State '{state.name}' is not locked")
    
    # Check if the lock ID matches or force is True
    if not force and state.lock_id != lock_id:
        raise ValueError(f"Cannot unlock state '{state.name}' with lock ID {lock_id}. It is locked with a different ID: {state.lock_id}")
